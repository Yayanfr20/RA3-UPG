...
$allLink = query("SELECT * FROM tugas_mk WHERE author = '$author'");
$jmlLink = count(query("SELECT * FROM tugas_mk WHERE author = '$author'"));
...
<body>
    ...
    <nav class="navbar shadow" style="background-color: #F08529;">
        <div class="container-fluid">
            <span class="navbar-brand mb-0 h1 text-white">Daftar Tugas</span>
                ...
                <?php foreach ($allLink as $link) : ?>
                    <div class="card text-dark mb-3 p-1" style="width: 16rem;">
                        <div class="card-body">
                            ...
                            <p class="card-title"><?= $link["code"]; ?></p>
                        </div>
                        <div class="card-footer">
                            <a href="delTugas.php?id=<?= $link['id']; ?>" onclick="return confirm('Yakin Anda Sudah Download Terlebih Dahulu?');" class="btn btn-sm btn-danger">Hapus</a>
                            <a href="https://api.whatsapp.com/send?text=Link%20Input%20Tugas%20http://ra3upg.rf.gd/InputTugas.php?id=<?= $link['id']; ?>%20%0AKode:<?= $link['code'];?>" class="btn btn-sm btn-success">Bagikan</a>
                            <a href="LihatTugas.php?id=<?= $link['id']; ?>" class="btn btn-sm btn-info text-white">Lihat</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php else : ?>
            <div class="d-flex justify-content-center">
                <div class="card text-dark mb-3 p-1 text-center" style="width: 16rem;">
                    <div class="card-body">
                        <h5 class="card-title">Tidak Ada Tugas</h5>
                    </div>
                    <div class="card-body">
                        <hr>
                    </div>
