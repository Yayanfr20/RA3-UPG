...
$allLink = query("SELECT * FROM list WHERE author = '$author'");
$jmlLink = count(query("SELECT * FROM list WHERE author = '$author'"));
            ...
            <div class="row d-flex justify-content-around">
                <?php foreach ($allLink as $link) : ?>
                    <div class="card text-dark mb-3 p-1" style="width: 16rem;">
                        <div class="card-body">
                            <h6 class="card-title"><?= $link["judul_list"]; ?></h6>
                            <p class="card-title"><?= $link["code"]; ?></p>
                        </div>
                        <div class="card-footer">
                            <a href="delList.php?id=<?= $link['id']; ?>" onclick="return confirm('Yakin Anda Sudah Cetak Terlebih Dahulu?');" class="btn btn-sm btn-danger">Hapus</a>
                            <a href="https://api.whatsapp.com/send?text=http://ra3upg.rf.gd/ReadList.php?id=<?= $link['id']; ?>" class="btn btn-sm btn-success">Bagikan</a>
                            <a href="CetakList.php?code=<?= $link['code']; ?>" class="btn btn-sm btn-info text-white">Cetak</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php else : ?>
            <div class="d-flex justify-content-center">
                <div class="card text-dark mb-3 p-1 text-center" style="width: 16rem;">
                    <div class="card-body">
                        <h5 class="card-title">Tidak Ada List</h5>
                    </div>
                    <div class="card-body">
                        <hr>
                    </div>
