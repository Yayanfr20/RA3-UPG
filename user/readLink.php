...
$mk = $user["mk"];
$allLink = query("SELECT * FROM absensi WHERE mk = '$mk'");
            ...
            <div class="row d-flex justify-content-around">
                <?php foreach ($allLink as $link) : ?>
                    <div class="card text-dark mb-3 p-1" style="width: 16rem;">
                        <div class="card-body">
                            <h6 class="card-title"><?= $link["mk"]; ?> - <?= $link["tanggal"]; ?></h6>
                            <p class="card-text">Kode absen : <?= $link["code"]; ?></p>
                        </div>
                        <div class="card-footer">
                            <a href="delAbsensi.php?id=<?= $link['id']; ?>" onclick="return confirm('Yakin Anda Sudah Cetak Terlebih DahulU?');" class="btn btn-sm btn-danger">Hapus</a>
                            <a href="https://api.whatsapp.com/send?text=*Absensi*%0A<?= $link['mk']; ?>-<?= $link['tanggal']; ?>%0A%0A*Kode%20Absen*%0A<?= $link['code']; ?>%0A%0A*Link*%0Ahttp://ra3upg.rf.gd/absenRA3.php?id=<?= $link['id']; ?>" class="btn btn-sm btn-success">Bagikan</a>
                            <a href="cetakAbsensi.php?code=<?= $link['code']; ?>" class="btn btn-sm btn-info text-white">Cetak</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php else : ?>
            <div class="d-flex justify-content-center">
                <div class="card text-dark mb-3 p-1 text-center" style="width: 16rem;">
                    <div class="card-body">
                        <h5 class="card-title">Tidak Ada Link</h5>
                    </div>
                    <div class="card-body">
                        <hr>
                    </div>
